...
def trucktimes(truck_id):
    ...
    res = requests.get(f"http://172.31.0.5:5000/item/{truck_id}", params=payload)
...
def totalbill(provider_id):
    ...
    if result:
        for truck in result:
            res = requests.get(f"http://172.31.0.5:5000/item/{truck[0]}", params=payload)
            if res.text != "No data found":
                truck_counter += 1
                sessions1 = res.json()
                ...
                for session in sessions:
                    r_session = requests.get(f"http://172.31.0.5:5000/session/{session}").json()
