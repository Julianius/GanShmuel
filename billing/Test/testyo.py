...
def checkprovider():
    URL = "http://localhost:8081/api/providers"
    # provider_id_in_db=[]
...
def checkrates():
    URL = "http://localhost:8081/api/rates"
    with open(f'testfile/testfile.txt', "r") as testfile:
        readtest = testfile.readlines()
...
def trucktest(provider_id):
        URL = "http://localhost:8081/api/trucks"
        with open(f'testfile/testfile.txt', "r") as testfile:
            readtest = testfile.readlines()
        for line in range(8, 10, 1):  # test for post trucks
            ...
            if test == 1:
                return "POST to http://localhost:8081/api/trucks test not good"
        # test for get rates
        rget = requests.get(url=URL)
        test = check(rget.text,"Please enter truck license plate and provider id:")
        if test == 1:
            return "get to http://localhost:8081/api/trucks test not good"
